...
{#if $tool === 'select'}
	<div
		class="pointer-events-none fixed top-0 border-2 border-blue-600 bg-blue-400 opacity-35"
		style="translate: {dragPos[0] - clickPos[0] > 0 ? clickPos[0] : dragPos[0]}px {dragPos[1] -
			clickPos[1] >
		0
			? clickPos[1]
			: dragPos[1]}px; width: abs({dragPos[0] - clickPos[0]}px); height: abs({dragPos[1] -
			clickPos[1]}px);"
	></div>
{/if}
